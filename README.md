...
启动软件后会显示“使用指导与默认案例”。关闭后也可以随时从顶部 `帮助 > 使用指导与默认案例` 重新打开。

1. 选择 2D 或 3D 默认案例，确认单位制、材料、截面、约束和荷载。
2. 在左侧模型树定位对象，在中部视口检查结构与载荷方向，在右侧属性面板编辑参数。
3. 点击“求解”或“本地求解”。二维案例会自动求解，三维案例会先进行模型校核再调用本地 Worker。
4. 通过结果类型、教学助手和结果表查看变形、内力、反力、推导过程与危险位置。
